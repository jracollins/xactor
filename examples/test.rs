...
struct QuickActor {
    parent: Option<Addr<SubscriberParent>>,
}

#[async_trait::async_trait]
impl Actor for QuickActor {
    async fn started(&mut self, ctx: &mut Context<Self>) -> Result<()> {
        println!("Quickactor  Started");
        self.parent = Some(SubscriberParent::new().await.start().await.unwrap());
        ctx.send_later(DropParent, Duration::from_secs(5));
        Ok(())
    }
}
...
#[async_trait::async_trait]
impl Handler<DropParent> for QuickActor {
    async fn handle(&mut self, _ctx: &mut Context<Self>, _msg: DropParent) {
        self.parent.take().unwrap().stop().await;
        self.parent = None;
    }
}
...
impl Handler<InitializeChildSubscribers> for SubscriberParent {
    async fn handle(&mut self, ctx: &mut Context<Self>, _msg: InitializeChildSubscribers) {
        ...
        let children_unstarted_actors_vec = dummy_ids.into_iter().map(move |id| {
            ...
            Subscriber::new(id, addr, ctx.address())
        });
